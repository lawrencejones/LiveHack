...
(function($) { 
    var JFMFS = function(element, options) {
        ...
        elem.html(
            ...
            "        <span class='jfmfs-title'>" + settings.labels.filter_title + " </span><input type='text' id='jfmfs-friend-filter-text' value='" + settings.labels.filter_default + "'/><button id=\"stage2send\" class=\"btn btn-primary\">Skip Invites</button>" +
        ...
        FB.api('/me/friends?fields=' + settings.friend_fields, function(response) {
            ...
            uninitializedImagefriendElements.bind('inview', function (event, visible) {
                if( $(this).attr('src') === undefined) {
                    $("img", $(this)).attr("src", "//graph.facebook.com/" + this.id + "/picture?type=small").addClass('pro-pic');
                }
                $(this).unbind('inview');
            });
        ...
        var init = function() {
            ...
            elem.find(".jfmfs-friend:not(.selected)").on(
                'hover', function (ev) {
